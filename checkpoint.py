...
class CheckPoint:
    def __recursive_proc(jax_params, infunc, directory, cnt = 0, recursive = 0):
        if isinstance(jax_params, list) or isinstance(jax_params, tuple):
            tmp = list(jax_params)
            for i in range(len(list(jax_params))):
                tmp[i], cnt = CheckPoint.__recursive_proc(jax_params[i], infunc, directory, cnt, recursive + 1)
    ...
    def __load_func(val, directory, cnt):
        return onp.load(os.path.join(directory, "{}.npy".format(cnt)), allow_pickle = True)
    
    def __save_func(val, directory, cnt):
        onp.save(os.path.join(directory, "{}.npy".format(cnt)), val)
        return val
    def __equal_func(val, directory, cnt):
        loaded_arr = onp.load(os.path.join(directory, "{}.npy".format(cnt)), allow_pickle = True)
        assert((val == loaded_arr).all())
    def __not_equal_func(val, directory, cnt):
        loaded_arr = onp.load(os.path.join(directory, "{}.npy".format(cnt)), allow_pickle = True)
        assert(not (val == loaded_arr).all())
    
    def load_params(sample_params, directory):
        return CheckPoint.__recursive_proc(   sample_params,
                                        CheckPoint.__load_func,
                                        directory)

    def save_params(params, directory):
        CheckPoint.__recursive_proc(  params,
                                CheckPoint.__save_func,
                                directory)
